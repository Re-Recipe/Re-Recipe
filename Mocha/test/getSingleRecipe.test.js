  ...
  it('Should retrieve a single recipe with the expected properties', function(done) {
    chai.request('http://localhost:8080')
      ...
      .end(function (err, res) {
        ...
        expect(res.body).to.have.property('ingredients').that.is.an('array').with.length.above(0);
        expect(res.body.ingredients[0]).to.have.property('name').that.is.a('string');
        expect(res.body.ingredients[0]).to.have.property('quantity').that.is.a('number');
        expect(res.body.ingredients[0]).to.have.property('unit').that.is.a('string');
        expect(res.body).to.have.property('directions').that.is.an('array').with.length.above(0);
        expect(res.body.directions[0]).to.have.property('step').that.is.a('string');
        done();
      });
  });
