...
async function populateDiscover() {
  try {
    ...
    const discoverCollection = db.collection("discover");  // Changed from 'recipes' to 'discover'

    // Insert discover documents, referencing the recipe contents
    const recipe1 = {
    ...
    await discoverCollection.insertMany([recipe1, recipe2, recipe3, recipe4, recipe5, recipe6, recipe7]);

    console.log("Recipes added to discover collection.");
  } catch (error) {
    console.error(error);
  } finally {
    await client.close();
