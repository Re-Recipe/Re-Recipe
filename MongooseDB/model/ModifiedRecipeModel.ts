...
class ModifiedRecipeModel {
  ...
  public createSchema() {
    const schemaDefinition: mongoose.SchemaDefinition<IRecipe> = {
      ...
      recipe_versions: [recipeContentsSchema], // this is recipe_contents
  ...
  public async createModel() {
    try {
      ...
      console.error(
        "Error connecting to MongoDB or initializing ModifiedRecipe model:",
        e
      );
    }
  }
  ...
  /**
   * Adds a new recipe version to the modified recipe object
   * @param user_ID
   * @param recipe_ID
   *
   *  */
  public async addRecipeVersionToList(
    response: any,
    personal_recipe_ID: string,
    newVersionData: Partial<IRecipe>
  ) {
    try {
      const existingRecipe = await this.model
        .findOne({ personal_recipe_ID })
        .exec();
      ...
      const newVersionNumber = existingRecipe.recipe_versions.length + 1;
      ...
      existingRecipe.recipe_versions.push(newVersion);

      // Save the new version to the db
      const updatedRecipe = await existingRecipe.save();
    } catch (e) {
      console.error("Failed to retrieve modified recipe:", e);
      response
        .status(500)
        .json({ error: "Failed to retrieve modified recipe" });
    }
  }
  /**
   ...
   */
  public async retrieveModifiedRecipe(
    response: any,
    personal_recipe_ID: string
  ) {
    try {
  ...
  public async updateRecipeIngredients(
    response: any,
    personal_recipe_ID: string,
    newIngredients: { name: string; quantity: number; unit: string }[]
  ) {
    try {
   ...
   * @param response - Response object to send updated data.
   */
  public async updateRecipeDirections(
    response: any,
    personal_recipe_ID: string,
    newDirections: { step: string }[]
  ) {
    try {
      const result = await this.model
        .findOneAndUpdate(
  ...
  public async deleteModifiedRecipe(response: any, personal_recipe_ID: string) {
    try {
      ...
      if (result.deletedCount && result.deletedCount > 0) {
        response.json({
          message: `Modified recipe ${personal_recipe_ID} deleted successfully.`,
          result,
        });
      } else {
        response.status(404).json({ error: "Modified recipe not found" });
  ...
  public async addNotes(
    ...
    try {
      ...
      response
        .status(500)
        .json({ error: "Failed to add notes to modified recipe" });
    }
  }
